import { TTT3D, WIN_LINES_3D } from '../rules/ttt-3d.js';
...
function getBestMove(state: TTT3DState, aiPlayer: Player): number {
  const legal = engine.getLegalMoves(state).map(m => (m.data as { cellIndex: number }).cellIndex);
  const opponent: Player = aiPlayer === 'X' ? 'O' : 'X';
  ...
  // Pick cell with best positional score
  let bestScore = -Infinity;
  let bestMove = legal[0]!;
  for (const ci of legal) {
    const result = engine.applyMove(state, { data: { cellIndex: ci } }, aiPlayer);
    if (!result.ok) continue;
    const s = scoreBoard((result.state as TTT3DState).board, aiPlayer);
